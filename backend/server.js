...
// --- mock auth middleware for admin routes (in production replace with real auth) ---
function mockAuth(req, res, next) {
  // If Authorization header present: set req.user accordingly (very simple)
  const auth = req.headers['authorization'];
  if (auth && auth.startsWith('Bearer ')) {
    // token format: Bearer ROLE:SUPER (example) - this is only for quick testing
    const token = auth.slice(7);
    const parts = token.split(':');
    req.user = { role: parts[0] || 'user', is_admin: parts[0]==='admin', is_superadmin: parts[1]==='SUPER' };
  } else {
    req.user = null;
  }
  next();
